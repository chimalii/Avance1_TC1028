...
'''
    ...
    # Agregué una fila vacía y/o columna vacía porque los índices comienzan en 
    # 0 para filas y columnas en matrices.
'''
    Matrices: Con los datos introducidos por el usuario sirven para...
    conversionesOp ----> buscar el valor para realizar la conversión
    unidades ----------> buscar el nombre de las unidades que debe introducir 
                         el usuario.
    conversionesLtt ---> buscar las unidades a las que se convirtió con datos 
...
'''
----------------------------- funciones y uso -------------------------------
Función inicial que despliega el menú principal de magnitudes que maneja la 
calculadora y el submenú para la magnitud escogida por el usuario.

Devuelve una lista donde el 1er número pertenece a la magnitud que se desea 
convertir y el 2do a la conversión que se desea realizar.
'''
...
'''
Usando la función menuPrincipal se obtiene una la lista, utilizada como un 
índice en las matrices donde ubicará el operador para realizar la conversión, 
las unidades que se deben pedir y las que devolverá.

Se piden las unidades a convertir y...
Hay algunos casos especiales, en la magnitud temperatura donde la conversión 
no se puede resolver con una multiplicación, para estas operaciones se tuvieron 
que agregar condicionales.

Imprime la conversión.

Finalmente mediante otra función (continuar(c)) imprime un mensaje para el 
usuario preguntando si este 
...
'''
Función principal:
Ejecuta las instrucciones de la función conversiones() que englobla 
menuPrincipal() y devuelve la respuesta de continuar, si decide continuar (1) 
vuelve a ejecutar conversiones y si no, imprime un mensaje de despedida.
'''
...
'''
------------------ Despliegue de código en consola ------------------------
'''
