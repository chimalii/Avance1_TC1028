...
def conPrefijos(u,opcion_2):
    prefijos = [ 1e-18 , 1e-15, 1e-12, 1e-9, 1e-6, 1e-3, 1e-2, 1e-1, 1e3, 1e6, 1e9, 1e12, 1e15, 1e18]
    letras = ["a","f","p","n","µ","m","c","d","k","M","G","T","P","E"]
    if opcion == 7:
        x = opcion_2
        cont = 0
        for opcion_2 in range(len(prefijos)):
            if cont < x:
                cont = cont+1
            else:
                # y = letras[opcion_2]
                prefijo = u * (prefijos[opcion_2])
                print (u,"m equivalen a",prefijo, letras[opcion_2]) 
                break
...
"""))
        ...
        print("""
Gracias por usar esta calculadora, buen día :)
""")
